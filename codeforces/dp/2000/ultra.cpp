...
int main() {
    ...
    ll t;
    cin >> t;
    precomputeFactorials(10000, mod);
    while (t--) {
        int n; 
        cin >> n; 
        ll res = 0;
        for (int size = 1 ; size <= n ; size ++) {
            ...
            // how many possible sets will this wcotain we
            if (size * 2 >= n) {
                
                // how many possible subsets of size are ther e w
                res = (res + (2 * size+ 1 ) * nCr(n, size)) % mod; 
                continue;
            }
            for (int meow = size + 1 ; meow <= 2 * size + 1 ; meow ++) {
                ll c =  (nCr(meow - 1 , meow - 1 - size) * nCr(n - meow,  2 * size  + 1  - meow)) % mod; 
                res = (res + (c * meow)) % mod; 
            }
        }
