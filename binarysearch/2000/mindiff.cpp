...
ll solve(vll& A, ll n, ll k) {
    ...
    rep(i,0,n) mp[A[i]] += 1;
    auto doLeft = [&]() {
        ll cnt = begin(mp)->second, diff = next(begin(mp))->first - begin(mp)->first;
        ll up = min(diff, k / cnt);
        mp[begin(mp)->first + up] += begin(mp)->second;
        mp.erase(begin(mp));
        k -= up * cnt;
    };
    auto doRight = [&]() {
        ll cnt = prev(end(mp))->second, diff = prev(end(mp))->first - prev(prev(end(mp)))->first;
        ll down = min(diff, k / cnt);
        mp[prev(end(mp))->first - down] += prev(end(mp))->second;
        mp.erase(prev(end(mp)));
        k -= down * cnt;
    };
    while(sz(mp) > 1 and k) {
        auto l = begin(mp), r = prev(end(mp));
        if(l->second < r->second and l->second <= k) doLeft();
        else if(r->second <= l->second and r->second <= k) doRight();
        else break;
    }
    return prev(end(mp))->first - begin(mp)->first;
}
int main() {
    Code By Sumfi
    precision(15)
