...
void dfs(unordered_map<ll,vll>& adj, unordered_map<ll,vpll>& Q, vll& time, ll u, vll& res) {
    if(Q.count(u)) {
        for(auto& [time, val] : Q[u]) {
            update(time, val);
        }
    }
    res[u] = query(MAX_N - 1) - query(time[u]-1);
    for(auto& v : adj[u]) dfs(adj,Q,time,v,res);
    if(Q.count(u)) {
        for(auto& [time, val] : Q[u]) {
            update(time, -val);
        }
...
vll solve(vall3 Q, ll n) {
    unordered_map<ll,vll> adj;
    unordered_map<ll,vpll> tq;
    vll inc{1};
    rep(i,0,n) {
        auto [op,x,y] = Q[i];
        if(op == 1) {
            adj[x-1].push_back(sz(inc));
            inc.push_back(i+2);
        }
        if(op == 2) {
            tq[x-1].push_back({i+2,y});
        }
    }
    vll res(sz(inc));
    dfs(adj,tq,inc,0,res);
    return res;
}
int main() {
    Code By Sumfi
