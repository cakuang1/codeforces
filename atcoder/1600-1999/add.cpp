...
struct Seg {
    ll mi,ma,sum,pos;
    Seg *left, *right;
    Seg(vll& A, ll l, ll r) : mi(l), ma(r), sum(0), pos(INF) {
        if(l^r) {
            ll m = l + (r - l) / 2;
            left = new Seg(A,l,m);
    ...
    ll sumQuery(ll l, ll r) {
        ...
        return left->sumQuery(l,r) + right->sumQuery(l,r);
    }
    ll posQuery(ll l, ll r) {
        if(l <= mi and ma <= r) return pos;
        if(l > ma or r < mi) return INF;
        return min(left->posQuery(l,r), right->posQuery(l,r));
    }
...
vll solve(vll& A, vll& B, vall3& Q) {
    Seg* aseg = new Seg(A,0,sz(A)-1);
    Seg* bseg = new Seg(B,0,sz(B)-1);
    vll res;
    rep(i,0,sz(Q)) {
        auto [op,idx,x] = Q[i];
        if(op == 1) aseg->update(idx-1,x);
        if(op == 2) bseg->update(idx-1,x);
        if(op == 3) {
            ll l = idx, r = x - 1, now = aseg->sumQuery(idx-1,idx-1);
            while(l <= r) {
                ll p = min(r+1,bseg->posQuery(l,r));
                if(p != l) {
                    now += aseg->sumQuery(l,p-1);
                    l = p;
                }
                if(l <= r) {
                    ll a = aseg->sumQuery(l,l), b = bseg->sumQuery(l,l);
                    now = max(now + a, now * b);
                    l++;
                }
            }
